            ...
            while(t <= 225)
            {
                ...
                if(dist != 0 && dist < MAX_DISTANCE && t <= 225)
                {
                    ...
                    for( int i = t; i <= 225 ; i++ )
                    {
                        ...
                        {
                            /* cout << "-> " << currentDist << " " << prevPoint.dist << endl; */
...
Core::graphic_thread( )
{
    ...
    ha_lidar_packet_ptr_->distance[110] =570.6;
    ha_lidar_packet_ptr_->distance[111] =571.6;
    ha_lidar_packet_ptr_->distance[112] =573.6;
    ha_lidar_packet_ptr_->distance[113] =568.6;
    ha_lidar_packet_ptr_->distance[114] =566.6;
    ha_lidar_packet_ptr_->distance[115] =566.6;
    ha_lidar_packet_ptr_->distance[116] =566.6;
    ha_lidar_packet_ptr_->distance[117] =566.6;
    ha_lidar_packet_ptr_->distance[118] =566.6;
    ha_lidar_packet_ptr_->distance[119] =566.6;
    ha_lidar_packet_ptr_->distance[120] =566.6;
	...
	while( !stopThreadAsked_ )
	{
        ...
        for(int i(0);i < obstacles.size();i++)
        {
            ...
            /* bg.w = abs(obstacles[i].x2-obstacles[i].x1); */
            /* bg.h = abs(obstacles[i].y2-obstacles[i].y1); */
            bg.w = 100;
            bg.h = 100;
            bg.x = static_cast<int>(400.0 + obstacles[i].x1);
            bg.y = static_cast<int>(400.0 - obstacles[i].y1);
